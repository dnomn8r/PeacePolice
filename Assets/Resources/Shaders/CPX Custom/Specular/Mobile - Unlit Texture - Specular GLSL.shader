Shader "CPX_Custom/Mobile/Specular/Unlit - Specular GLSL" {
   Properties {
      ...
      Pass {    
         ...
         void main(){
            lowp vec4 textureColor = texture2D(_MainTex, uv);
         	gl_FragColor = vec4(textureColor.rgb+(specular.rgb*specular.a),1.0);         
         }
         #endif
         ENDGLSL
      }
   }
   Fallback "CPX_Custom/Mobile/Specular/Unlit - Specular CG"
}