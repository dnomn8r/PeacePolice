Shader "CPX_Custom/Mobile/Reflection/Reflection (Cubemap) CG" {
   Properties {
   ...
   SubShader {
      Pass {   
         ...
         pixelOutput pixelPass(vertexOutput input){
            ...
            fixed3 reflectedDirection = reflect(normalize(input.viewDirection), normalize(input.normalDirection));
         	output.color = color + (texCUBE(_Cubemap, reflectedDirection) * _ReflectStrength);  
         	return output;       
         }
         ENDCG
      }
