Shader "CPX_Custom/Mobile/Color Selection/Rim/Unlit Texture - 1 Color Selection Rim GLSL" {
   Properties {
   ...
   SubShader {
      Pass {    
         ...
         uniform lowp vec4 _Color1, _RimColor, _RimDirection;
         ...
         void main(){
            ...
            //viewDirection = WorldSpaceViewDir(gl_Vertex);
         }
         #endif
         #ifdef FRAGMENT
         void main(){
            lowp vec4 textureColor = texture2D(_MainTex, uv);
         	...
         	else{ finalColor = (_Color1.rgb)*textureColor.rgb;}
